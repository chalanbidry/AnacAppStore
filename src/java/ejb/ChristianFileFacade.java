 ...
 */
@Stateless
public class ChristianFileFacade extends AbstractFacade<ChristianFile> {

    Logger logger = Logger.getLogger(ChristianFileFacade.class);
  ...
  private GedFacade gedFacade;
    ...
    public ChristianFileFacade() {
        super(ChristianFile.class);
    }
    ...
    @Override
    public void create(ChristianFile entity) {
        try {
...
//            entity.setFileName("" + entity.getTypeDocument().getId() + "N° " + levelDocumentByType + "_" + JsfUtil.convertDate(new Date(), "EEEEE dd MMM yyyy 'A' HH:mm:ss"));
    ...
    public void verrouillerDocument(ChristianFile documentCourant) throws IOException {
...
//        Query jpql = em.createQuery("SELECT d, max(d.progressionDossier.dateCreation) FROM Document d WHERE  d.progressionDossier.annee=:annee AND d.progressionDossier.statut.code NOT IN ('ARC') GROUP BY d.progressionDossier.dossier.id,d.id,d.progressionDossier.dateCreation HAVING  max(d.progressionDossier.dateCreation)=d.progressionDossier.dateCreation");
     ...
     */
//    A revoir
    public List<ChristianFile> getListeChhristianFilInCategorie(CategorieChristianFile categorie) {
        List<ChristianFile> documents;
        Query jpql = em.createQuery("SELECT d FROM ChristianFile d WHERE  d.categorie=:categorie");
        jpql.setParameter("categorie", categorie);
        documents = jpql.getResultList();
        if (documents == null) {
            documents = new ArrayList<>();
        }
